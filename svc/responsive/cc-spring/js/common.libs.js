...
var CCS = CCS || {};
CCS.UI = CCS.UI || {};
CCS.VIEWPORT_WIDTH = null;
CCS.VIEWPORT_HEIGHT = null;
CCS.IS_MOBILE = false;
CCS.IS_VIEWTYPE = null;
CCS.IS_IE8 = ( $('html').hasClass('ie8') ) ? true : false;
CCS.IS_SIZE = CCS.IS_SIZE || {};
CCS.IS_SIZE.MAXMOBILE = 768;
CCS.IS_SIZE.MAXTABLET = 1023;
CCS.IS_TABLET = false;
CCS.FOCUS_ELEM = null;

CCS.DELAY_FUNC = (function(){
    var timer = 0;
...
var SetViewSize = function(){
    CCS.VIEWPORT_WIDTH = Math.max(document.documentElement.clientWidth, window.innerWidth || 0);
    CCS.VIEWPORT_HEIGHT = Math.max(document.documentElement.clientHeight, window.innerHeight || 0);
};
...
var DeivceChkFn= function() {
    if(CCS.VIEWPORT_WIDTH < CCS.IS_SIZE.MAXMOBILE && CCS.IS_MOBILE){
        CCS.IS_VIEWTYPE = 'mobile';
    } else if(CCS.VIEWPORT_WIDTH <= CCS.IS_SIZE.MAXTABLET && CCS.IS_MOBILE){
        CCS.IS_VIEWTYPE = 'tablet';
    } else {
        if(CCS.VIEWPORT_WIDTH < CCS.IS_SIZE.MAXMOBILE ) {
            CCS.IS_VIEWTYPE = 'mobile';
        } else if (CCS.VIEWPORT_WIDTH <= CCS.IS_SIZE.MAXTABLET ) {
            CCS.IS_VIEWTYPE = 'tablet';
        } else {
            CCS.IS_VIEWTYPE = 'web';
        }
...
CCS.UI.ResponsiveImageSrc = function($elems, device){
    "use strict";
    var _this = this;
    _this.elem = $elems;
    _this.initFlag = true;
