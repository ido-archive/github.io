...
;(function($,window,document,undefined) {
    ...
    (function(pluginName){
        ...
        var defaults = {
            'item' : '.retinaimg',
            'maxMobile' : CCS.IS_SIZE.MAXMOBILE || 767 ,
            'maxTablet' :  CCS.IS_SIZE.MAXTABLET || 1023
        };
            ...
            return this.each(function(){
                ...
                this.RetinaReplace = function(){
                    $item.each(function (index) {
                        var $this = $(this);
                        if(CCN.IS_VIEWTYPE != 'pc' && window.devicePixelRatio != undefined) { // 일반적인 pc사용시

//				if(window.devicePixelRatio != undefined) { // 맥북 레티나 고려시
                            if ($this.data(CCN.IS_VIEWTYPE).indexOf('@') != -1) {
                                //if (window.devicePixelRatio >= 4) {
                                //	imgSrc[index] = $this.data(viewType).replace(/\@1x/g, '@4x');
                                //} else /* 4x 이미지 필요시 활성화 */
                                if(window.devicePixelRatio >= 3) {
                                    imgSrc[index] = $this.data(CCN.IS_VIEWTYPE).replace(/\@1x/g, '@3x');
                                } else if (window.devicePixelRatio >= 2) {
                                    imgSrc[index] = $this.data(CCN.IS_VIEWTYPE).replace(/\@1x/g, '@2x');
                                } else if (window.devicePixelRatio >= 1 ) {
                                    imgSrc[index] = $this.data(CCN.IS_VIEWTYPE);
                                }
                            } else {
                                imgSrc[index] = $this.data(CCN.IS_VIEWTYPE);
                            }
                ...
                this.ChangeBandImgSrc = function(){
                    if (_var.initFlag || _var.oldViewType != CCN.IS_VIEWTYPE) {
                        var $img = [];
                        $item.each(function (index) {
                        ...
                        _var.initFlag = false;
                        _var.oldViewType = CCN.IS_VIEWTYPE;
                    }
                };
